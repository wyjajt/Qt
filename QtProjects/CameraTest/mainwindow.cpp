...
static int          save_pic_number = 0;    //保存图片编号
...
//    sprintf( filename, "%stest", dir );
    ...
    QByteArray pic_name = QString("%1%2%3")
                          .arg( path ).arg("photo_snap_")
                          .arg( QString::number( save_pic_number) ).toLatin1();
    dir = pic_name.data();
...
//软触发下保存图片
void MainWindow::saveImageInSoftMode()
{
    ...
    if( CameraGetImageBuffer( g_hCamera, &tFrameHead, &pbyBuffer, 1000) == CAMERA_STATUS_SUCCESS )
    {
        ...
        case FILE_JPG:
        {
            ...
            */
            CameraImageProcess( g_hCamera, pbyBuffer, pbImgBuffer, &tFrameHead );
            //界面显示图片
            QImage img( pbImgBuffer, g_W_H_INFO.sensor_width, g_W_H_INFO.sensor_height, QImage::Format_Indexed8 );
