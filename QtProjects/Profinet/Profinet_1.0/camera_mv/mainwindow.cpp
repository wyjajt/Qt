...
//    Log_Debug( logger, "GetImageBuffer time: %s ", QDateTime::currentDateTime().toString("hh-mm-ss,zzz").toLocal8Bit().data()  );
    //获得一帧图像数据保存到buffer中
    Log_Debug( logger, "enter CameraGetImageBuffer ..." );
    if( CameraGetImageBuffer( g_hCamera, &tFrameHead, &vec_buffer.at( buffer_index ).pbyBuffer, 1000 ) == CAMERA_STATUS_SUCCESS )
    {
        switch( g_SaveImage_type )
        {
        case FILE_JPG:
        ...
        case FILE_BMP:
        {
            TIME_NOW( time_imgpro_start );
            timeList_imgpro_start << time_imgpro_start;

            //处理图像数据
            CameraImageProcess( g_hCamera, vec_buffer.at( buffer_index ).pbyBuffer, vec_buffer.at( buffer_index ).pbImgBuffer, &tFrameHead );

            TIME_NOW( time_imgpro_end );
            timeList_imgpro_end << time_imgpro_end;

            delay_camera( ui->lineEdit->text().toInt() );

            //释放由CameraGetImageBuffer获得的缓冲区
            CameraReleaseImageBuffer( g_hCamera, vec_buffer.at( buffer_index ).pbImgBuffer );
        ...
        case FILE_RAW:
        {
            ...
            if( type == Type_Start )        //继续开启线程拍照
            {
                rcv_takePhoto( Type_Start );
            }

            data.detect_result = (int)ImageAnalysis( vec_buffer.at( buffer_index ).pbyBuffer,
                           g_tCapability.sResolutionRange.iHeightMax * g_tCapability.sResolutionRange.iWidthMax /*3*/ , 128 );

            QImage img( vec_buffer.at( buffer_index ).pbyBuffer, g_W_H_INFO.sensor_width, g_W_H_INFO.sensor_height, QImage::Format_Indexed8/*Format_RGB888*/ );

            //释放由CameraGetImageBuffer获得的缓冲区
            CameraReleaseImageBuffer( g_hCamera, vec_buffer.at( buffer_index ).pbyBuffer );
            vec_buffer.at( buffer_index ).is_idle = true;   //buffer置为空闲状态
            img.setColorTable( grayColourTable );
            //界面触发或复检时显示图片
            if( type == Type_OnlyOnce )
...
void MainWindow::rcv_takePhoto( int type )
{
    ...
    if( type == Type_Start )
    {
        ...
        timeList_imgpro_start.clear();
        timeList_imgpro_end.clear();
...
//    Log_Debug( logger, "rcvActionInfo time:  %s ", QDateTime::currentDateTime().toString("hh-mm-ss,zzz").toLocal8Bit().data()  );
...
void MainWindow::caculateTime()
{
    ...
    for( int i = 0; i < size; i++ )
    {
        trigger   += timeList_imgpro_start.at(i)  -  timeList_trigger_start.at(i) ;
        imgpro    += timeList_imgpro_end.at(i)    -  timeList_imgpro_start.at(i)  ;
        newthread += timeList_analyse_start.at(i) -  timeList_imgpro_end.at(i)    ;
        analyse   += timeList_qimg_start.at(i)    -  timeList_analyse_start.at(i) ;
        qimg      += timeList_qimg_end.at(i)      -  timeList_qimg_start.at(i)    ;

        max_index_trigger   = max_trigger   < ( timeList_imgpro_start.at(i)  -  timeList_trigger_start.at(i) ) ? i : max_index_trigger  ;
        max_index_imgpro    = max_imgpro    < ( timeList_imgpro_end.at(i)    -  timeList_imgpro_start.at(i)  ) ? i : max_index_imgpro   ;
        max_index_newthread = max_newthread < ( timeList_analyse_start.at(i) -  timeList_imgpro_end.at(i)    ) ? i : max_index_newthread;
        ...
        max_trigger   = max_trigger   < ( timeList_imgpro_start.at(i)  -  timeList_trigger_start.at(i) ) ? ( timeList_imgpro_start.at(i)  -  timeList_trigger_start.at(i) ) : max_trigger  ;
        max_imgpro    = max_imgpro    < ( timeList_imgpro_end.at(i)    -  timeList_imgpro_start.at(i)  ) ? ( timeList_imgpro_end.at(i)    -  timeList_imgpro_start.at(i)  ) : max_imgpro   ;
        max_newthread = max_newthread < ( timeList_analyse_start.at(i) -  timeList_imgpro_end.at(i)    ) ? ( timeList_analyse_start.at(i) -  timeList_imgpro_end.at(i)    ) : max_newthread;
    ...
    Log_Debug( logger, "trigger    ==>  %d / %d = %f", (int)trigger    , timeList_imgpro_start.size()  , (double)trigger   / size );
    Log_Debug( logger, "imgpro     ==>  %d / %d = %f", (int)imgpro     , timeList_imgpro_start.size()  , (double)imgpro    / size );
    Log_Debug( logger, "newthread  ==>  %d / %d = %f", (int)newthread  , timeList_imgpro_end.size()    , (double)newthread / size );
    ...
    Log_Debug( logger, "max_imgpro     ==>  %d", (int)max_imgpro    );
    ...
    Log_Debug( logger, "max_index_imgpro     ==>  %d", max_index_imgpro    );
