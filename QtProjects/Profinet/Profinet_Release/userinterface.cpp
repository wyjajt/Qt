...
void UserInterface::rcvActionInfo( int action )
{
    ...
    {
        if( action == Action_Rough_Detect )
        {
            ui->lineEdit_max_value_rough->setText( "NULL");
            ui->lineEdit_position_rough->setText( "NULL" );
        }
        ...
        action_state != Action_Fine_MoveOver && action == Action_Fine_MoveOver )
    {
        ...
        if( action == Action_Rough_MoveOver )
        {
            Log_Debug( logger, "rcvActionInfo ----> Rough_MoveOver ..." );
        }
        else
        {
            Log_Debug( logger, "rcvActionInfo ----> Fine_MoveOver ..." );
        }
    ...
    if( action_state != Action_ReDetect && take_photo_end && action == Action_ReDetect )
    {
        ...
        if( ui->lineEdit_redetect_dalay->text() == "" )
        {
            delay_ui( 20 );
        }
        else
        {
            delay_ui( ui->lineEdit_redetect_dalay->text().toInt() );
        }
...
void UserInterface::thd_getMaxResult()
{
    ...
    for(;;)
    {
        if( take_photo_end && action_state == Action_Fine_MoveOver )
        {
            Log_Debug( logger, "write result position ==> FineDetect" );
        ...
        else if( take_photo_end && action_state == Action_Rough_MoveOver )
        {
            //粗扫光通量和坐标
            Log_Debug( logger, "write result position ==> RoughDetect : %d = ( %d, %d )", detectResult, result_pos_x_1, result_pos_y_1 );
            ui->lineEdit_max_value_rough->setText( QString::number(detectResult));
            ui->lineEdit_position_rough->setText( QString("( %1, %2 )")
                                              .arg( QString::number(result_pos_x_1) )
                                              .arg( QString::number(result_pos_y_1) ) );
...
//        ui->tableWidget->item( tableRowCount, 3 )->setText( QString::number( abs( pos_y[2] - pos_y[0] ) ) );
        ui->tableWidget->item( tableRowCount, 2 )->setText( QString::number( abs( data.detect_result - detectResult ) ) );
