...
static auto delay_camera = AccurateDelay_ms();
    ...
    ui(new Ui::MainWindow), m_scene(0), m_image_item(0)
{
    ...
    initLog( logger, LOG_NAME_ROOT, LOG_NAME_CAMERA );
    ...
    status   = 1;
    stop_take_photo = false;
    image_size = QSize(480,320);
    initVector();
}
...
MainWindow::~MainWindow()
{
    ...
    Log_Debug( logger, "~MainWindow succcess ..." );
...
void MainWindow::setPLC(PLC_Adi *argPLC)
{
    plc = argPLC;
}
...
void MainWindow::camera_statues()
{
    m_camera_statuesFps->setText( QString("Capture fps: %1  Display fps :%2" )
                                  .arg( QString::number(g_read_fps, 'f', 2) )
                                  .arg( QString::number(g_disply_fps, 'f', 2) )
                                  );
    g_read_fps   = 0;
    g_disply_fps = 0;
}
...
void MainWindow::Image_process( QImage img )
{
    ...
    //    m_scene->setSceneRect(0, 0, 600, 480);
...
int MainWindow::GUI_init_parameter( int hCamera, tSdkCameraCapbility *pCameraInfo )
{
    ...
    ui->snap_path_lineEdit->setText( "./" );

    g_SaveImage_type = FILE_BMP;
...
int  MainWindow::GUI_init_Resolution( int hCamera, tSdkCameraCapbility *pCameraInfo )
{
    ...
    for( int i=0; i < iImageSizeDesc; i++ )
    {
...
{
    int size = pCameraInfo->iFrameSpeedDesc;

    for( int i=0; i < size; i++ )
    {
        radioButton_speed[i] = new QRadioButton( QString::fromLocal8Bit( pCameraInfo->pFrameSpeedDesc[i].acDescription ), ui->groupBox_speed );
    ...
    if( CameraSnapToBuffer( g_hCamera, &tFrameHead, &pbyBuffer, 1000) == CAMERA_STATUS_SUCCESS )
    {
        ...
        case FILE_JPG:
        {
            ...
            */
            ...
            //将图像缓冲区的数据保存成图片文件
            CameraSaveImage( g_hCamera, filename, pbImgBuffer, &tFrameHead, FILE_JPG, 100 );
        ...
        default:
            qDebug( "save type error: %d", g_SaveImage_type );
            break;
        }
    }
    else
...
{
    g_SaveImage_type = FILE_JPG;
}
//保存图片格式 png
void MainWindow::on_radioButton_png_clicked()
{
    g_SaveImage_type = FILE_PNG;
}
//保存图片格式 bmp
...
//    CameraSoftTrigger( g_hCamera );
    ...
    //保存图片
    saveImageInSoftMode( Type_OnlyOnce );
}

//软触发下保存图片
void MainWindow::saveImageInSoftMode( int type )
{
    ReadData data;
    int buffer_index = -1;
    ...
    if( stop_take_photo )
    {
        Log_Debug( logger, "enter stop loop ...");
        ...
        for(;;)
        {
            ...
            for( int i = 0; i < vec_buffer.size(); i++ )
            {
                ...
                is_idle = vec_buffer.at( i ).is_idle && is_idle;
            }

            if( is_idle )
            {
                Log_Debug( logger, "all takePhoto threads have completed ... ");
                emit takePhotoAllOver();
    ...
    {
        //存在空闲的buffer
        if( vec_buffer.at(i).is_idle )
        {
            buffer_index = i;
            //            Log_Debug( logger, "use ImageBuffer %d...", i );
            vec_buffer.at( buffer_index ).is_idle = false;
//            memset( vec_buffer);
            break;
        }
    }
    ...
    if( buffer_index == -1 )
    {
        //创建新的buffer
        addBufferToVector();
        buffer_index = vec_buffer.size() - 1;
        vec_buffer.at( buffer_index ).is_idle = false;
    }

    tSdkFrameHead  tFrameHead;

    data.pos_x_1 = readData.pos_x_1;
    data.pos_y_1 = readData.pos_y_1;
    //执行一次软触发, 执行后, 会触发由CameraSetTriggerCount指定的帧数
    CameraSoftTrigger( g_hCamera );
    data.pos_x_2 = plc->data.pos_x_1;
    data.pos_y_2 = plc->data.pos_y_1;

    TIME_NOW( time_buffer_start );
    timeList_buffer_start << time_buffer_start;

    //获得一帧图像数据保存到buffer中
//    Log_Debug( logger, "enter CameraGetImageBuffer ..." );
    if( CameraGetImageBuffer( g_hCamera, &tFrameHead, &g_pRawBuffer, 1000 ) == CAMERA_STATUS_SUCCESS )
    {
        switch( g_SaveImage_type )
        {
        case FILE_JPG:
        ...
        case FILE_BMP:
        {
            TIME_NOW( time_buffer_end );
            timeList_buffer_end << time_buffer_end;

            //处理图像数据
            CameraImageProcess( g_hCamera, g_pRawBuffer, vec_buffer.at( buffer_index ).pbImgBuffer, &tFrameHead );

            CameraReleaseImageBuffer( g_hCamera, g_pRawBuffer );

            if( type == Type_Start || type == Type_Continue )   //继续开启线程拍照
            {
                rcv_takePhoto( Type_Continue );
            }

            TIME_NOW( time_analyse_start );
            timeList_analyse_start << time_analyse_start;

            data.detect_result = (int)ImageAnalysis( vec_buffer.at( buffer_index ).pbImgBuffer,
                                                     g_tCapability.sResolutionRange.iHeightMax * g_tCapability.sResolutionRange.iWidthMax /*3*/ , 128 );

            TIME_NOW( time_qimg_start );
            timeList_qimg_start << time_qimg_start;

            QImage img( vec_buffer.at( buffer_index ).pbImgBuffer, g_W_H_INFO.sensor_width, g_W_H_INFO.sensor_height, QImage::Format_Indexed8/*Format_RGB888*/ );

            vec_buffer.at( buffer_index ).is_idle = true;   //buffer置为空闲状态
            img.setColorTable( grayColourTable );
            //界面触发或复检时显示图片
            if( type == Type_OnlyOnce )
            {
                Image_process( img );
            }

            TIME_NOW( time_qimg_end );
            timeList_qimg_end << time_qimg_end;

            emit sendImageOut( data.toQString(), img.scaled( image_size, Qt::IgnoreAspectRatio) );
            break;
        }
        ...
        case FILE_RAW:
        {
            ...
            data.detect_result = (int)ImageAnalysis( g_pRawBuffer,
                                                     g_tCapability.sResolutionRange.iHeightMax * g_tCapability.sResolutionRange.iWidthMax /*3*/ , 128 );

            TIME_NOW( time_qimg_start );
            timeList_qimg_start << time_qimg_start;

            QImage img( g_pRawBuffer, g_W_H_INFO.sensor_width, g_W_H_INFO.sensor_height, QImage::Format_Indexed8/*Format_RGB888*/ );
...
//复检时保存图片
void MainWindow::saveReDetectImage()
{
    Log_Debug( logger, "enter saveReDetectImage ..." );
    ...
    for( int i = 0; i < 4; i++ )
    {
        ...
        CameraSoftTrigger( g_hCamera );
//        Log_Debug( logger, "enter CameraGetImageBuffer ..." );
        if( CameraGetImageBuffer( g_hCamera, &tFrameHead, &pbyBuffer, 1000) == CAMERA_STATUS_SUCCESS )
        {
            switch( g_SaveImage_type )
            {
            case FILE_JPG:
            ...
            case FILE_BMP:
            {
                ...
                //将图像缓冲区的数据保存成图片文件
                CameraSaveImage( g_hCamera, filename, pbImgBuffer, &tFrameHead, g_SaveImage_type, 100 );

                result[i] = (int)ImageAnalysis( pbImgBuffer,
                                                         g_tCapability.sResolutionRange.iHeightMax * g_tCapability.sResolutionRange.iWidthMax /*3*/ , 128 );

                Log_Debug( logger, "value : %d", result[i] );
                QImage img( pbImgBuffer, g_W_H_INFO.sensor_width, g_W_H_INFO.sensor_height, QImage::Format_Indexed8/*Format_RGB888*/ );
                result_img = img.copy();    //QImage深拷贝
                Image_process( img );
            ...
            case FILE_RAW:
            {
                CameraSaveImage( g_hCamera, filename, pbyBuffer, &tFrameHead, g_SaveImage_type, 100);

                result[i] = (int)ImageAnalysis( pbyBuffer,
                                                         g_tCapability.sResolutionRange.iHeightMax * g_tCapability.sResolutionRange.iWidthMax /*3*/ , 128 );
    ...
    data.detect_result = result[0];
    for( int i = 1; i < 4; i++ )
    {
        if( result[i] >  data.detect_result )
        {
            data.detect_result = result[i];
        }
    }

    emit sendImageOut( data.toQString(), result_img.scaled( image_size, Qt::IgnoreAspectRatio) );
}

long MainWindow::ImageAnalysis( uchar *img, int bufferSize, BYTE threshold )
{
    long  analysis = 0;
...
//接收PLC发送的动作信息
void MainWindow::rcv_takePhoto( int type )
{
    //不是软触发模式
    if( pbySnapMode != 1 )
    {
    ...
    {
        take_photo_times = 0;
        timeList_trigger_start.clear();
        timeList_buffer_start.clear();
        timeList_buffer_end.clear();
        timeList_analyse_start.clear();
        timeList_qimg_start.clear();
        timeList_qimg_end.clear();
    }

    if( type == Type_Stop )
    {
        Log_Debug( logger, "rcv stop take photo ... ");
        stop_take_photo = true;
        return;
    }

    //触发前取坐标
    readData.pos_x_1 = plc->data.pos_x_1;
    readData.pos_y_1 = plc->data.pos_y_1;

    TIME_NOW( time_trigger_start );
    timeList_trigger_start << time_trigger_start;

    if( type == Type_ReDtect )
    {
        saveReDetectImage();
        return;
    }

//    Log_Debug( logger, "Camera start take photo ==== times: %d", take_photo_times++ );
    //创建软触发线程
    std::thread thd_soft_trigger;
    thd_soft_trigger = std::thread( &MainWindow::saveImageInSoftMode, this, type );
    thd_soft_trigger.detach();
}

// 判定光通量算法函数
// 参数m_szheight：图像的高
//       m_szWidth：图像的宽
//       m_szChannels：图像的位深，默认为3（当前默认是24位RGB）
//       m_szpFrameBuffer：图像buffer指针
//       m_szThreshold：光通量判定阈值，0~255的值(固定128)
LONG __stdcall MainWindow::CA_ContourAreaSimple( int m_szheight,
                                                 int m_szWidth,
                                                 int m_szChannels,
                                                 unsigned char* m_szpFrameBuffer,
                                                 BYTE m_szThreshold )
{
    int		channels = m_szChannels;
    int		step = m_szWidth*channels;

    LONG	m_AreaAlgorithm = 0;

    for (int i = 0; i < m_szheight; i++)
    {
        for (int j = 0; j < m_szWidth; j++)
        {
            BYTE m_szBit = m_szpFrameBuffer[ i * step + j * channels ];

            if( m_szBit > m_szThreshold )
            {
                m_AreaAlgorithm++;
            }
        }
...
void MainWindow::initVector()
{
    ...
    for( int i = 0; i < 4; i++ )
    {
        ImgBuffer buffer( 1280, 1024, 1 );
        vec_buffer.push_back( buffer );
    }
}

void MainWindow::addBufferToVector()
{
    Log_Debug( logger, "add Buffer ...");
    ImgBuffer buffer( 1280, 1024, 1 );
    vec_buffer.push_back( buffer );
}

void MainWindow::caculateTime()
{
    Log_Debug( logger, "enter time caculate ..." );
    ...
    long max_trigger   = 0;
    long max_getbuffer = 0;
    long max_newthread = 0;
    long max_analyse   = 0;
    long max_qimg      = 0;

    int  max_index_trigger   = 0;
    int  max_index_buffer    = 0;
    int  max_index_newthread = 0;
    int  max_index_analyse   = 0;
    int  max_index_qimg      = 0;

    int size = timeList_qimg_end.size();
    for( int i = 0; i < size; i++ )
    {
        trigger   += timeList_buffer_start.at(i)  -  timeList_trigger_start.at(i) ;
        getbuffer += timeList_buffer_end.at(i)    -  timeList_buffer_start.at(i)  ;
        newthread += timeList_analyse_start.at(i) -  timeList_buffer_end.at(i)    ;
        analyse   += timeList_qimg_start.at(i)    -  timeList_analyse_start.at(i) ;
        qimg      += timeList_qimg_end.at(i)      -  timeList_qimg_start.at(i)    ;

        max_index_trigger   = max_trigger   < ( timeList_buffer_start.at(i)  -  timeList_trigger_start.at(i) ) ? i : max_index_trigger  ;
        max_index_buffer    = max_getbuffer < ( timeList_buffer_end.at(i)    -  timeList_buffer_start.at(i)  ) ? i : max_index_buffer   ;
        max_index_newthread = max_newthread < ( timeList_analyse_start.at(i) -  timeList_buffer_end.at(i)    ) ? i : max_index_newthread;
        max_index_analyse   = max_analyse   < ( timeList_qimg_start.at(i)    -  timeList_analyse_start.at(i) ) ? i : max_index_analyse  ;
        max_index_qimg      = max_qimg      < ( timeList_qimg_end.at(i)      -  timeList_qimg_start.at(i)    ) ? i : max_index_qimg     ;

        max_trigger   = max_trigger   < ( timeList_buffer_start.at(i)  -  timeList_trigger_start.at(i) ) ? ( timeList_buffer_start.at(i)  -  timeList_trigger_start.at(i) ) : max_trigger  ;
        max_getbuffer = max_getbuffer < ( timeList_buffer_end.at(i)    -  timeList_buffer_start.at(i)  ) ? ( timeList_buffer_end.at(i)    -  timeList_buffer_start.at(i)  ) : max_getbuffer;
        max_newthread = max_newthread < ( timeList_analyse_start.at(i) -  timeList_buffer_end.at(i)    ) ? ( timeList_analyse_start.at(i) -  timeList_buffer_end.at(i)    ) : max_newthread;
        max_analyse   = max_analyse   < ( timeList_qimg_start.at(i)    -  timeList_analyse_start.at(i) ) ? ( timeList_qimg_start.at(i)    -  timeList_analyse_start.at(i) ) : max_analyse  ;
        max_qimg      = max_qimg      < ( timeList_qimg_end.at(i)      -  timeList_qimg_start.at(i)    ) ? ( timeList_qimg_end.at(i)      -  timeList_qimg_start.at(i)    ) : max_qimg     ;
    }

    Log_Debug( logger, "trigger    ==>  %d / %d = %f", (int)trigger    , timeList_buffer_start.size()  , (double)trigger   / size );
    Log_Debug( logger, "getbuffer  ==>  %d / %d = %f", (int)getbuffer  , timeList_buffer_start.size()  , (double)getbuffer / size );
    Log_Debug( logger, "newthread  ==>  %d / %d = %f", (int)newthread  , timeList_buffer_end.size()    , (double)newthread / size );
    Log_Debug( logger, "analyse    ==>  %d / %d = %f", (int)analyse    , timeList_analyse_start.size() , (double)analyse   / size );
    Log_Debug( logger, "qimg       ==>  %d / %d = %f", (int)qimg       , timeList_qimg_start.size()    , (double)qimg      / size );

    Log_Debug( logger, "max_trigger    ==>  %d", (int)max_trigger   );
    Log_Debug( logger, "max_getbuffer  ==>  %d", (int)max_getbuffer );
    Log_Debug( logger, "max_newthread  ==>  %d", (int)max_newthread );
    Log_Debug( logger, "max_analyse    ==>  %d", (int)max_analyse   );
    Log_Debug( logger, "max_qimg       ==>  %d", (int)max_qimg      );

    Log_Debug( logger, "max_index_trigger    ==>  %d", max_index_trigger   );
    Log_Debug( logger, "max_index_buffer     ==>  %d", max_index_buffer    );
    Log_Debug( logger, "max_index_newthread  ==>  %d", max_index_newthread );
    Log_Debug( logger, "max_index_analyse    ==>  %d", max_index_analyse   );
    Log_Debug( logger, "max_index_qimg       ==>  %d", max_index_qimg      );
}
